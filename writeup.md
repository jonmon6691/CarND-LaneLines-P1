...
|-------------------|---|
| ![solidWhiteCurve_Step00_original] Original Image |The pipeline presented consists of 7 steps that take a frame from the input and detect two lanes in a specific region of interest and output the image with two lines drawn on top that indicate the detected line. An example input frame is given to the left. |
